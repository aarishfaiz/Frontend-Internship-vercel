...
const ShowJobs = () => {
  ...
  const applyInternship = async () => {
    try {
      ...
      await axios.post(
        "hhttps://intern-backend-fop1.onrender.com/api/application",
        applicationData
      );
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredData.map((job) => (
          <div key={job.id} className="bg-white p-4 rounded-lg shadow-lg">
            ...
            <p className="text-gray-600 mb-2">
              <span className="font-semibold">CTC:</span> {job.CTC}
            </p>
            <p className="text-gray-600 mb-2">
            ...
            <button onClick={() => openPopup(job)} className="bg-blue-600 font-semibold text-white p-2 rounded-lg">
              Apply
            </button>
          </div>
        ))}
